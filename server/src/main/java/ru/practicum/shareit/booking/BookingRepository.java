...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query("SELECT b.end FROM Booking b WHERE b.item.id = :itemId AND b.status = 'APPROVED' AND b.end < :now ORDER BY b.end DESC LIMIT 1")
    LocalDateTime findLastBookingEndDate(@Param("itemId") Long itemId, @Param("now") LocalDateTime now);

    @Query("SELECT b.start FROM Booking b WHERE b.item.id = :itemId AND b.status = 'APPROVED' AND b.start > :now ORDER BY b.start ASC LIMIT 1")
    LocalDateTime findNextBookingStartDate(@Param("itemId") Long itemId, @Param("now") LocalDateTime now);
}